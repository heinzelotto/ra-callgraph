// copied from rust-analyzer source
...
use rust_analyzer::reload::{load_proc_macro, ProjectFolders, SourceRootConfig};
...
pub fn load_it(path: &Path) -> (AnalysisHost, vfs::Vfs, Option<ProcMacroServer>) {
    ...
    return load_workspace_at(path, &cargo_config, &load_cargo_config, &|_| {}).unwrap();
